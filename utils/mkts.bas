' mkts v 0.5.8 - Converts several kinds of tilesets
' Copyleft 2015, 2016 The Mojon Twins
...
Sub usage
	...
	Puts "           [deinterlaced] [allbg] [aseprite] [simple] [silent]"
	Puts ""
...
Sub nesDoTiles (img As Any Ptr, pal () As Integer, xOrg As Integer, yOrg As Integer, wProc As Integer, hProc As Integer, wMeta As Integer, hMeta As Integer, tsMapFn As String, label As String, max As Integer, fOut As Integer)
	...
	For y = y0 To y1 Step vSize
		...
		Next x
		If wMeta = 2 And hMeta = 2 Then Print #fOut, ""
		If max <> -1 And ctt = max Then Exit For
	Next y
	...
	For y = y0 To y1 Step vSize
		...
		For x = x0 To x1 Step hSize
			...
			If wMeta = 2 And hMeta = 2 Then 
				...
				Else
					If tsMapFn <> "" Then 
						Print #fOut, "0x" & Lcase (Hex (wp, 2)); 'Ltrim (Str (wp));
			...
			ElseIf wMeta = 4 And hMeta = 4 Then
				...
				'filteredPuts ("BYTE: " & Hex (i, 2))
				If deinterlaced Then 
					byteArrays (16, tMapsIndex) = i
				Else
					If tsMapFn <> "" Then 
						Print #fOut, "0x" & Lcase (Hex (i, 2)); 'Ltrim (Str (i));
						If y < y1+1-vSize Or x < x1+1-hSize Then Print #fOut, ", ";
					End If
				End If
			End If
	...
	While Not Eof (fIn)
		...
		Select Case cmd
			...
			Case "MAPFILE":
				...
				Print #fMapFile, "// Copyleft 2016 The Mojon Twins"
				Print #fMapFile, ""
			Case "LABEL":
				label = tokens (1)
...
filteredPuts "mkts v0.5.8 20161102"
filteredPuts "Cuts & imports tilesets from a png."
filteredPuts "Copyleft 2015, 2016 The Mojon Twins"
filteredPuts ""
